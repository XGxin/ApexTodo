...
- 极简桌面挂件：无边框圆角窗口，仅驻留系统托盘（不显示任务栏），低干扰使用。
- 浅色 / 深色主题：支持浅色、深色与跟随系统，顶栏一键切换。
...
- 在设置面板开启“显示 Codex 用量”后，顶部会显示 5 小时与本周用量、进度和重置时间。
- 该功能默认关闭；开启后每 5 分钟自动刷新，也可点击顶部刷新按钮手动更新。
- 需要当前电脑已登录 Codex。登录凭据只在 Electron 主进程中读取，不会传入页面或写入日志。

### 外观主题

- 顶栏左侧的圆点按钮可在浅色 / 深色之间快速切换。
- 设置面板中可选「跟随系统」，自动匹配 Windows 的深浅色外观。

### 系统托盘

- ApexTodo 运行时不显示任务栏按钮，只在系统托盘保留图标。
- 点击托盘图标可恢复并聚焦主窗口；托盘菜单提供打开设置、切换鼠标穿透和退出入口。
